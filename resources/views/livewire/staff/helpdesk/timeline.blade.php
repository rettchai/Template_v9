    ...
    <x-modal.card title="Timeline&Chat" blur wire:model.defer="isModelOpen">
        @if (!empty($get_helpdesksDetails))
            ...
            {{-- @if ($hdd->getHelpdesk->StatusText != 'close') --}}
                <div class="row">
                    <div class="col-md-12">
                        <x-native-select label="Select Status" wire:model="statuss">
                            {{-- <option>Active</option> --}}
                            @foreach ($refStatus as $statuss)
                                <option value="{{ $statuss->HelpdeskStatusEN }}">{{ $statuss->HelpdeskStatusEN }}
                                </option>
                            @endforeach
                        ...
                        <x-input wire:model="Comments" class="border-2" label="Chat.."
                            wire:keydown.enter="onClickChat()"></x-input>
